...
export function PageHeader({
  ...
  return (
    <header className="flex flex-col gap-4 border-b pb-5 xl:flex-row xl:items-end xl:justify-between" style={{ borderColor: "var(--negis-border)" }}>
      <div className="min-w-0">
        {kicker ? (
          <p className="text-xs font-semibold uppercase tracking-[0.08em]" style={{ color: "var(--negis-primary)" }}>
            {kicker}
          </p>
        ) : null}
        <h1 className="mt-1 text-2xl font-semibold leading-tight" style={{ color: "var(--negis-text)" }}>
          {title}
        </h1>
        {description ? (
