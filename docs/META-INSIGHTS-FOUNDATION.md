...
### Сводка для владельца в рекламном центре

`/ads` использует тот же безопасный агрегат `meta-insights-history`, но только
после отдельной серверной проверки текущей Supabase-сессии и роли `owner/admin`
для активного workspace. Клиентский экран не доверяет локальному переключателю
режима, не сохраняет Insights в localStorage и не делает запрос для demo
workspace. Пользователь без подтверждённых прав видит только нейтральное
объяснение доступности отчёта.

Сводка показывает фактический расход Meta, показы, клики, клики по ссылке и
Meta leads по уже синхронизированным кампаниям. Расход суммируется как bigint в
minor units отдельно для каждой пары currency/exponent. Валюты не смешиваются,
reach не агрегируется, технические идентификаторы и ответы Meta не выводятся.
Пустой успешный период объясняется отдельно от ошибки и от ещё не выполненной
синхронизации. Это факты доставки рекламы, а не расчёт CPL, ROI, ROMI или оценка
эффективности.
