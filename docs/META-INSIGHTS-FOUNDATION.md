...
Рядом показан отдельный «Результат в CRM». Он загружает production-
заявки и продажи только после той же server-verified проверки
`owner/admin`; demo/localStorage данные не подмешиваются. Связанная
заявка или продажа определяется только по наличию
`meta_campaign_launch_id`. В выручку входят только связанные продажи со
статусом `paid`; pending/cancelled/refunded записи деньги не увеличивают.
Валюты остаются раздельными. Это ручная CRM-атрибуция, а не доказанная
причинная связь с рекламой; она не используется для автоматических выводов
или изменений кампании.
