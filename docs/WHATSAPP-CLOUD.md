...
- **Подпись**: каждый POST проверяется по `X-Hub-Signature-256` —
  ...
  до какого-либо парсинга. Entry point читает именно сырые байты потока
  событийным API и не трогает `req.body`: хелперы @vercel/node возвращают
  буферизованные байты в поток (`restoreBody`), а пере-сериализованный JSON —
  не то, что подписывала Meta (`\uXXXX`-эскейпы кириллицы не совпадут).
  Нет заголовка / не совпал — 401 без деталей. БД недоступна — 503,
  Meta ретраит сама.
...
1. **Применить миграции 027 и 028** в Supabase SQL Editor: вставить
   содержимое `migrations/027_whatsapp_cloud_numbers.sql`, затем
   `migrations/028_leads_phone_normalized.sql` целиком и выполнить.
   028 нужна обоим провайдерам: без неё дедуп повторных обращений работает
   через старый обход (до 1000 открытых заявок в клинике — дальше возможен
   дубль). Код это переживает и сам переключится на точный поиск, как только
   миграция применена, — простоя между деплоем и миграцией нет.
