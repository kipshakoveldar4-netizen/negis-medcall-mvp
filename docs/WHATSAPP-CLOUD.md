...
После шага 6 номер появляется в Настройки → **WhatsApp** рядом с каналом
Wazzup, если он есть: оба провайдера показаны одним списком, потому что для
клиники это один канал обращений. Там же счётчики заявок и выключатель приёма.
Пока миграция 027 не применена, прямое подключение показано как «ещё не
активировано» — это не ошибка чтения, и рабочий канал Wazzup продолжает
отображаться.
