...
test("MP18 скрипт заливки не отдаёт услуги наугад и не занижает цены", async () => {
  // Двух Дильназ различает специальность — по ней скрипт и выбирает карточку.
  // Если и специальность не различает, строки пропускаются и мастер называется
  // в отчёте: найл-услуги у визажиста — это неверный чек клиента.
  const sql = await readFile(path.join(repoRoot, "migrations", "data", "salon-prices-2026-08-20.sql"), "utf8");
  assert.ok(/position\(spec_key in specialty\) > 0/.test(sql), "специальность различает тёзок");
  assert.ok(/\) = 1;/.test(sql), "и заливка идёт только при однозначном совпадении");
  assert.ok(/and d\.is_active/.test(sql), "архивная карточка не считается вторым мастером");
