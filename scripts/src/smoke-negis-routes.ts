...
async function checkContentStudioPhaseOne() {
  ...
  for (const marker of [
    ...
    "ИИ-провайдер не подключён: показан образец, а не результат генерации.",
    "checkMetaCompliance",
  ...
  for (const marker of [
    ...
    'parseAdvertisingCampaignPrefillForPlatform(JSON.parse(raw), "meta")',
    'incoming.sourceModule !== "advertising-hub"',
    "setAdvertisingGoalContext(context)",
    "advertisingGoalContext.targetPatients !== undefined",
    "city: hasAdvertisingGoalContext ? packageBrief.city : undefined",
    "Генерация начнётся только после вашего нажатия.",
    'platform: "meta"',
    'sourceKind: "photo"',
...
async function checkLeadsPageSource() {
  ...
  for (const marker of [
    ...
    "Создать клиента",
    "Клиент уже создан",
    // Кнопка ведёт на КАРТОЧКУ созданного клиента, а не в общий список:
    // регистратор искал его там глазами среди сотен. Пин ходил за старой
    // подписью «Открыть клиентов» и держал смоук красным.
    "Открыть клиента",
    "rememberOpenClient",
    "convertLeadToClient",
...
async function checkLayoutFoundation() {
  ...
  for (const marker of [
    ...
    "Необработанные лиды",
    "Записи сегодня",
    // Подпись строится из словаря вертикали: у клиники «Пациенты», у салона
    // «Клиенты». Литерал «Пациенты для повторного визита» исчез из исходника
    // вместе с этим переходом и держал смоук красным, маскируя настоящие
    // регрессии. Пиним конструкцию, а не одну из её форм.
    "${capitalize(terms.customerPlural)} для повторного визита",
...
async function checkAdvertisingHubSource() {
  ...
  for (const marker of [
    ...
    "createAdvertisingCampaignPrefill",
    "ADVERTISING_CONTENT_STUDIO_PREFILL_KEY",
    'prepareCampaign("/content-studio")',
