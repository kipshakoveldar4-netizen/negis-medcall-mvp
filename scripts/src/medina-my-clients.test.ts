...
test("MC9 во вкладке «Клиенты» есть выбор мастера, и он не выдумывает базу сам", async () => {
  const page = await read("artifacts", "negis", "src", "pages", "ClientsPage.tsx");

  // База мастера спрашивается у того же маршрута, что и страница «База
  // клиентов». Считать её на клиенте (например, по записям в списке) значило бы
  // завести второе правило видимости, которое разъедется с серверным.
  assert.match(page, /crmFetch\(`\/api\/crm\/my-clients\?\$\{query\.toString\(\)\}`\)/);
  assert.ok(page.includes('<option value="all">Все клиенты салона</option>'), "выбор мастера обязан быть на экране");
  assert.match(page, /if \(doctorClientIds && !doctorClientIds\.has\(client\.id\)\) return false;/);

  // Отказ чтения не превращается в «у мастера нет клиентов»: по такому списку
  // решают, кому отдать нового клиента.
  assert.ok(page.includes("Это сбой связи, а не пустая база — не раздавайте клиентов заново."));
  assert.ok(page.includes("База мастера показана не целиком"), "обрезка обязана быть видна");
});
